import os
from flask import Flask, session, redirect, jsonify, url_for, request, flash, render_template
...
def create_connection():
    ...
    try:
        connection = mysql.connector.connect(
            ...
            password='prince',  # Your MySQL password
            database='db_theatre'  # Your MySQL database name
        )
...
def recom():
    ...
    query = "SELECT movie_title,genres,title_year,imdb_score FROM movies; "  # Adjust the query as needed
    movies = runQuery(query)
    
    
    if movies is None:
        print("Failed to fetch movies from the database.")
        return "Internal Server Error", 500
    random.shuffle(movies)
    # Pass the fetched data to the template
    return render_template('recom.html', movies=movies[:9])
